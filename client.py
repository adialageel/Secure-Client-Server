...
def main():
    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as sock:
        ...
        print('Sending AS_REQ message to authentication server ...')
        sock.sendto(msg_pack, (HOST_AS, PORT_AS))

        # Receive AS_RES message
        response1 = sock.recvfrom(MAX_LEN)
        print('Received AS_RES message from the authenticatoin server.')
        ...
        kcs = unpacked[0]
        print('Decrypted the message with Kc and took out the session key (Kcs).')
        tkt = unpacked[5]
        ...
        while type == APP_DATA:
            ...
            f.write(chunk)
            print('Wrote chunk to file. size={}'.format(len))
        ...
        # TERMINATE packet
        if type == TERMINATE:
            print('This is the TERMINATE packet.')
            s = struct.Struct('! I I')
