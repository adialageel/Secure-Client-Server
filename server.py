    ...
    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as sock:
        ...
        while True:
            ...
            if is_ap_req(message):
                ...
                print('Sending AP_RES message to client ...')
                sock.sendto(resp_enc, address)

                continue

            if is_app_data_req(message):
                print('Received APP_DATA_REQUEST message.')
                # Decrypt the message with Kcs
                msg_dec = decrypt(kcs, message)
                ...
                print('Beginning to transfer the file ...')
                crcvalue = 0
                with open(in_fpath, 'rb') as f:
                    ...
                    while len(chunk) > 0:
                        ...
                        print('Encrypting the packet with session key(Kcs) ...')
                        pack_enc = encrypt(kcs, packet)

                        print('Sending the packet to client ...')
                ...
                print('Sending the TERMINATE packet ...')
                sock.sendto(pack_enc, address)
            else:
